...
// initSession establishes a session with FINVIZ by visiting the homepage first
func (s *FinvizScraper) initSession() error {
	...
	// Just read and discard the body to complete the request
...
func (s *FinvizScraper) Scrape(queryURL string) (*ScrapeResult, error) {
	...
	// Initialize session by visiting homepage first to get cookies
	if err := s.initSession(); err != nil {
		// Session init failed but continue anyway - might still work
		_ = err
	}
	...
	findTickers = func(n *html.Node) {
		...
		if n.Type == html.ElementNode && n.Data == "a" {
			for _, attr := range n.Attr {
				// FINVIZ ticker links have href="quote.ashx?t=TICKER" (no leading slash)
