...
func main() {
	...
	refreshBtn := widget.NewButton("Load from Backend", func() {
		...
		if equity, ok := settings["equity"]; ok {
			equityLabel.SetText(fmt.Sprintf("Equity: $%.2f", equity))
		}
		if riskPct, ok := settings["risk_pct"]; ok {
			riskLabel.SetText(fmt.Sprintf("Risk %%: %.2f%%", riskPct*100))
		}
		if portfolioCap, ok := settings["portfolio_heat_cap"]; ok {
			portfolioCapLabel.SetText(fmt.Sprintf("Portfolio Cap: %.2f%%", portfolioCap))
		}
		if bucketCap, ok := settings["bucket_heat_cap"]; ok {
			bucketCapLabel.SetText(fmt.Sprintf("Bucket Cap: %.2f%%", bucketCap))
		}
